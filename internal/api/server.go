...
type Server struct {
	...
	log           *slog.Logger
	version       string
	downloadHosts []AllowedHost
}

// NewServer creates a Server wired with the given clients, API key, version string, logger, and download hosts.
func NewServer(nfsClient nfs.Client, apiKey, version string, log *slog.Logger, downloadHosts []AllowedHost) *Server {
	return &Server{
		nfs:           nfsClient,
		apiKey:        apiKey,
		log:           log,
